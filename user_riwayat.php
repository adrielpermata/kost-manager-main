...
$sql = "(SELECT bulan_tagihan as info, jatuh_tempo as tanggal, jumlah_tagihan as nominal, 'Tagihan' as tipe, status_tagihan as status 
         FROM tagihan WHERE id_penghuni = '$id_penghuni')
        UNION
        (SELECT no_invoice as info, tgl_bayar as tanggal, jumlah_bayar as nominal, 'Pembayaran' as tipe, 'Selesai' as status 
         FROM pembayaran WHERE id_penghuni = '$id_penghuni')
...
<head>
    ...
    <title>Riwayat Aktivitas - KostManager</title>
    ...
    <div class="main-wrapper">
        ...
        <main class="main-content">
            <div class="data-section">
                <h1 style="margin-top:0; font-size: 24px; color: #1e293b;">Log Aktivitas Keuangan</h1>
                <p style="color: #64748b; margin-bottom: 20px;">Ringkasan seluruh transaksi tagihan dan pembayaran Anda.</p>
                        ...
                        <tbody>
                            <?php if(mysqli_num_rows($result) > 0): ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if($row['tipe'] == 'Pembayaran'): ?>
                                            <span style="background:#e0e7ff; color:#4338ca; padding:5px 10px; border-radius:6px; font-size:12px; font-weight:bold;"><i class="fas fa-arrow-up"></i> Pembayaran Masuk</span>
                                        <?php else: ?>
                                            <span style="background:#fef3c7; color:#d97706; padding:5px 10px; border-radius:6px; font-size:12px; font-weight:bold;"><i class="fas fa-arrow-down"></i> Tagihan Keluar</span>
                                        <?php endif; ?>
                                    </td>
                                    <td><?= htmlspecialchars($row['info']) ?></td>
                                    <td><strong><?= formatRupiah($row['nominal']) ?></strong></td>
                                </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="4" style="text-align:center; padding:40px; color:#94a3b8;">
                                        <i class="fas fa-history" style="font-size:30px; margin-bottom:10px; display:block;"></i>
